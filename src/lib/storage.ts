...
/**
 * Get signed URL for ID proof image
 * Only accessible by authenticated admin/staff users
...
export async function getIdProofImageUrl(path: string): Promise<string | null> {
  ...
  try {
    ...
    console.error('Error accessing ID proof image:', error);
    return null;
  }
}

/**
 * Get multiple signed URLs for ID proof images
 * Used for batch operations like police export
 */
export async function getIdProofImageUrls(paths: string[]): Promise<Map<string, string>> {
  const urlMap = new Map<string, string>();
  
  await Promise.all(
    paths.map(async (path) => {
      if (path) {
        const url = await getIdProofImageUrl(path);
        if (url) {
          urlMap.set(path, url);
        }
      }
